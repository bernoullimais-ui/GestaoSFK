...
const Relatorios: React.FC<RelatoriosProps> = ({ alunos, turmas, presencas, matriculas = [], experimentais = [] }) => {
  ...
  // Filtros BI Conversão (Período de Aula Experimental)
  ...
  const statsConversao = useMemo(() => {
    ...
    if (filtroUnidadeBI) {
      filtered = filtered.filter(e => normalizeText(e.unidade) === normalizeText(filtroUnidadeBI));
    }
    ...
    experimentais.forEach(e => {
      ...
      if (!chartDataMap[key]) {
        chartDataMap[key] = { label: monthLabel, agendamentos: 0, matriculas: 0 };
      }
  ...
  }, [experimentais, filtroUnidadeBI, conversaoDataInicio, conversaoDataFim]);
  ...
  const statsFluxo = useMemo(() => {
    ...
    if (filtroUnidadeBI) {
      filteredTurmas = turmas.filter(t => normalizeText(t.unidade) === normalizeText(filtroUnidadeBI));
    }
    ...
    filteredTurmas.forEach(t => {
      stats[t.id] = { id: t.id, turma: t.nome, professor: t.professor, unidade: t.unidade, inicio: 0, novas: 0, cancelados: 0, fim: 0 };
    });
    
    matriculas.forEach(m => {
      const tObj = filteredTurmas.find(t => 
        normalizeText(t.unidade) === normalizeText(m.unidade) && 
        (normalizeText(m.turmaId).includes(normalizeText(t.nome)) || normalizeText(t.id) === normalizeText(m.turmaId))
      );
      ...
      if (dMat && dMat < start) {
        s.inicio++;
      } else if (dMat && dMat >= start && dMat <= end) {
        s.novas++;
      }
    });
  ...
  }, [matriculas, alunos, turmas, fluxoDataInicio, fluxoDataFim, filtroUnidadeBI]);

  const getCursosDoAluno = (alunoId: string) => {
    return matriculas
      .filter(m => m.alunoId === alunoId)
      .map(m => {
        const t = turmas.find(t => t.id === m.turmaId || normalizeText(t.nome) === normalizeText(m.turmaId.split('-')[0]));
        if (t) return t.nome;
        return m.turmaId.split('-')[0].trim();
      })
      .filter(Boolean);
  };
  ...
  const statsSecretaria = useMemo(() => {
    return alunos.filter(aluno => {
      ...
      if (filtrosCursosSec.length > 0) {
        const matchCurso = cursosAluno.some(ca => 
          filtrosCursosSec.some(fs => normalizeText(ca).includes(normalizeText(fs)) || normalizeText(fs).includes(normalizeText(ca)))
        );
  ...
  const statsFrequenciaGeral = useMemo(() => {
    ...
    presencas.forEach(pres => {
      const studentNameFromRecord = (pres as any)._estudantePlanilha || pres.alunoId;
      const classNameFromRecord = (pres as any)._turmaPlanilha || pres.turmaId;
      const unitFromRecord = pres.unidade;
      ...
      if (freqEstudante) {
        const selAluno = alunos.find(a => a.id === freqEstudante);
        if (selAluno && normalizeText(selAluno.nome) !== normalizeText(studentNameFromRecord)) return;
      }
      
      if (freqUnidade && normalizeText(unitFromRecord) !== normalizeText(freqUnidade)) return;
      
      if (freqTurma) {
        const selTurma = turmas.find(t => t.id === freqTurma);
        if (selTurma && normalizeText(selTurma.nome) !== normalizeText(classNameFromRecord)) return;
      }
      
      const key = `${pres.data}|${normalizeText(unitFromRecord)}|${normalizeText(classNameFromRecord)}`;
      if (!grouped[key]) {
        let tema = "";
        if (pres.observacao) {
          const match = pres.observacao.match(/\[Aula: (.*?)\]/);
          if (match) tema = match[1];
          else if (!pres.observacao.includes("[Aluno:")) tema = pres.observacao;
        }
        grouped[key] = { data: pres.data, unidade: unitFromRecord, turma: classNameFromRecord, p: 0, f: 0, tema: tema };
      }
      if (pres.status === 'Presente') grouped[key].p++;
      else grouped[key].f++;
    });
    return Object.values(grouped).sort((a, b) => b.data.localeCompare(a.data));
  }, [presencas, turmas, alunos, freqEstudante, freqTurma, freqUnidade, freqDataInicio, freqDataFim]);
  ...
  const handleExportCSV = () => {
    ...
    if (activeTab === 'frequencia_geral') {
      ...
      rows = statsFrequenciaGeral.map(row => [
        parseToDate(row.data)?.toLocaleDateString('pt-BR') || '',
        row.unidade,
        row.turma,
        row.p.toString(),
        row.f.toString(),
        `${Math.round((row.p / (row.p + row.f)) * 100)}%`,
        row.tema || ''
      ]);
      ...
      if (biSubTab === 'fluxo') {
        ...
        rows = statsFluxo.data.map(row => [
          row.turma,
          row.unidade,
          row.inicio.toString(),
          row.novas.toString(),
          row.cancelados.toString(),
          row.fim.toString(),
          `${row.inicio > 0 ? Math.round(((row.fim - row.inicio) / row.inicio) * 100) : 100}%`
        ]);
        ...
        rows = statsConversao.filteredRaw.map(e => [
          e.estudante,
          e.unidade,
          e.curso,
          parseToDate(e.aula)?.toLocaleDateString('pt-BR') || '',
          e.responsavel1 || '',
          cleanPhone(e.whatsapp1),
          e.status || 'Pendente',
      ...
      rows = statsSecretaria.map(aluno => [
        ...
        aluno.responsavel1 || '', 
        cleanPhone(aluno.whatsapp1), 
        aluno.responsavel2 || '', 
        cleanPhone(aluno.whatsapp2), 
        aluno.email || ''
      ]);
    }

    const csvContent = [headers, ...rows]
      .map(r => r.map(field => `"${(field || '').toString().replace(/"/g, '""')}"`).join(';'))
      .join('\n');
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-slate-100 shadow-sm">
                ...
                <select value={filtroUnidadeBI} onChange={e => setFiltroUnidadeBI(e.target.value)} className="bg-transparent border-none rounded-lg text-[10px] font-black uppercase outline-none cursor-pointer">
              ...
              {biSubTab === 'conversao' && (
                <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-slate-100 shadow-sm animate-in fade-in zoom-in-95">
                  <Calendar className="w-3.5 h-3.5 text-blue-500" />
                  <span className="text-[9px] font-black text-slate-400 uppercase">Período Aula:</span>
                  <input type="date" value={conversaoDataInicio} onChange={e => setConversaoDataInicio(e.target.value)} className="bg-slate-50 border-none rounded-lg text-[10px] font-bold outline-none px-2 py-1" />
                  <span className="text-slate-300">até</span>
                  <input type="date" value={conversaoDataFim} onChange={e => setConversaoDataFim(e.target.value)} className="bg-slate-50 border-none rounded-lg text-[10px] font-bold outline-none px-2 py-1" />
                </div>
              )}

              {biSubTab === 'fluxo' && (
                <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-slate-100 shadow-sm">
                  <span className="text-[9px] font-black text-slate-400 uppercase">Análise:</span>
                      ...
                      {[
                        ...
                        return (
                          <div key={idx} className="flex items-center gap-8">
                            ...
                            <div className="flex-1 h-14 bg-slate-50 rounded-2xl overflow-hidden relative group">
                               <div 
                                 className={`h-full ${item.color} transition-all duration-1000 ease-out flex items-center justify-end pr-8`}
            ...
            <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-700">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 relative overflow-hidden group">
                   ...
                   <p className="text-[10px] font-black text-slate-400 uppercase mt-6">Ativos no início do período</p>
                </div>
                <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 relative overflow-hidden group">
                   ...
                   <p className="text-[10px] font-black text-emerald-500 uppercase mt-6">Entradas no período</p>
                </div>
                <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 relative overflow-hidden group">
                   ...
                   <p className="text-[10px] font-black text-red-400 uppercase mt-6">Saídas no período</p>
                </div>
                <div className="bg-blue-600 p-10 rounded-[40px] shadow-xl relative overflow-hidden group text-white">
                   ...
                   <p className="text-[10px] font-black text-blue-100 uppercase mt-6">Posição final da unidade</p>
                </div>
              </div>
              ...
              <div className="bg-[#1e1b4b] rounded-[40px] shadow-2xl overflow-hidden border border-white/10">
                 <div className="px-10 py-8 flex items-center justify-between">
                    ...
                    <div className="text-right">
                       <p className="text-[10px] font-black text-indigo-300 uppercase tracking-widest">TOTAL DE {statsFluxo.data.length} CURSOS</p>
                    </div>
                 </div>
                 ...
                 <div className="overflow-x-auto">
                    <table className="w-full text-left">
                       ...
                       <tbody className="divide-y divide-white/5 bg-white">
                          {statsFluxo.data.length > 0 ? statsFluxo.data.map((row) => {
                             ...
                             return (
                                <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                                   <td className="px-10 py-6">
                                      <p className="text-sm font-black text-slate-800 uppercase leading-none">{row.turma}</p>
                                   </td>
                                   <td className="px-10 py-6 text-center">
                                      <span className="text-lg font-black text-slate-500">{row.inicio}</span>
                                   </td>
                                   <td className="px-10 py-6 text-center">
                                      <span className="text-lg font-black text-emerald-500">+{row.novas}</span>
                                   </td>
                                   <td className="px-10 py-6 text-center">
                                      <span className="text-lg font-black text-red-400">-{row.cancelados}</span>
                                   </td>
                                   <td className="px-10 py-6 text-center">
                                      <span className="text-xl font-black text-blue-600">{row.fim}</span>
                                   </td>
                                   <td className="px-10 py-6 text-right">
                                      <div className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl font-black text-sm border ${
                             ...
                             <tr>
                                <td colSpan={6} className="px-10 py-20 text-center text-slate-400 italic font-bold">Nenhuma turma localizada para os critérios de filtro.</td>
                             </tr>
                          )}
                       </tbody>
                    </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
              <div className="md:col-span-3">
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-1 tracking-widest flex items-center gap-2">
                  <User className="w-3 h-3 text-indigo-500"/> BUSCA RÁPIDA
                </label>
              ...
              <div className="md:col-span-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-1 tracking-widest flex items-center gap-2">
                  <MapPin className="w-3 h-3 text-blue-500"/> UNIDADE
                </label>
                <select 
                  value={filtroUnidadeSec} 
                  onChange={e => { setFiltroUnidadeSec(e.target.value); setFiltrosCursosSec([]); }} 
                  className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-blue-500 outline-none font-bold text-sm appearance-none cursor-pointer"
                >
              ...
              <div className="md:col-span-3 relative">
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-1 tracking-widest flex items-center gap-2">
                  <GraduationCap className="w-3 h-3 text-emerald-500"/> TURMAS (MULTI-ESCOLHA)
                </label>
                <button 
                  onClick={() => setIsCoursesOpen(!isCoursesOpen)}
                  className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl flex items-center justify-between font-bold text-sm text-slate-600 hover:border-slate-200 transition-all"
                >
                  <span className="truncate">{filtrosCursosSec.length === 0 ? 'Todas as Turmas' : `${filtrosCursosSec.length} selecionadas`}</span>
                      ...
                      <div className="flex flex-col gap-2">
                        {turmasDisponiveisSec.map(t => (
                          <label key={t} className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-2xl cursor-pointer transition-colors group">
                            <input 
                              type="checkbox" 
                              checked={filtrosCursosSec.includes(t)} 
                              onChange={() => setFiltrosCursosSec(prev => prev.includes(t) ? prev.filter(x => x !== t) : [...prev, t])}
                              className="w-5 h-5 rounded-lg border-2 border-slate-200 text-blue-600 focus:ring-blue-500" 
                            />
                            <span className="text-[11px] font-black text-slate-600 uppercase group-hover:text-blue-600">{t}</span>
                          </label>
                        ))}
                      </div>
                    </div>
              ...
              <div className="md:col-span-4">
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-1 tracking-widest flex items-center gap-2">
                  <Activity className="w-3 h-3 text-blue-500"/> STATUS DE MATRÍCULA
                </label>
                  ...
                  {statsSecretaria.length > 0 ? statsSecretaria.map(aluno => (
                    <tr key={aluno.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-6">
                        <p className="text-xs font-black text-slate-800 uppercase">{aluno.nome}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase">{aluno.unidade}</p>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-wrap gap-1">
                          {getCursosDoAluno(aluno.id).map((c, i) => (
                            <span key={i} className="px-2 py-1 bg-blue-50 text-blue-600 text-[9px] font-black uppercase rounded">{c}</span>
                          ))}
                        </div>
                      </td>
                      <td className="px-8 py-6 text-xs font-bold">{aluno.responsavel1 || '--'}</td>
                      <td className="px-8 py-6 text-xs">{cleanPhone(aluno.whatsapp1)}</td>
                      <td className="px-8 py-6 text-xs font-bold">{aluno.responsavel2 || '--'}</td>
                      <td className="px-8 py-6 text-xs">{cleanPhone(aluno.whatsapp2)}</td>
                      <td className="px-8 py-6 text-xs truncate max-w-[150px]">{aluno.email || '--'}</td>
                    </tr>
                  )) : (
                    <tr>
                      <td colSpan={7} className="px-8 py-20 text-center text-slate-400 italic font-bold">Nenhum contato localizado.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
           <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-6 items-end">
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest flex items-center gap-2"><User className="w-3.5 h-3.5" /> ESTUDANTE</label>
                    <select value={freqEstudante} onChange={e => setFreqEstudante(e.target.value)} className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-blue-500 outline-none font-bold text-slate-700">
                 ...
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest flex items-center gap-2"><MapPin className="w-3.5 h-3.5" /> UNIDADE</label>
                    <select value={freqUnidade} onChange={e => { setFreqUnidade(e.target.value); setFreqTurma(''); }} className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-blue-500 outline-none font-bold text-slate-700">
                       <option value="">Todas as Unidades</option>
                       {unidadesUnicas.map(u => <option key={u} value={u}>{u}</option>)}
                    </select>
                 </div>
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest flex items-center gap-2"><GraduationCap className="w-3.5 h-3.5" /> TURMA</label>
                    <select 
                       value={freqTurma} 
                       onChange={e => setFreqTurma(e.target.value)} 
                       disabled={!freqUnidade}
                       className={`w-full px-6 py-4 border-2 rounded-2xl focus:border-blue-500 outline-none font-bold text-slate-700 transition-all ${!freqUnidade ? 'bg-slate-50 border-slate-50 text-slate-300' : 'bg-slate-50 border-slate-100'}`}
                    >
                       <option value="">{freqUnidade ? 'Todas as Turmas' : 'Selecione Unidade'}</option>
                       {turmasFiltradasFreq.map(t => <option key={t.id} value={t.id}>{t.nome}</option>)}
                    </select>
                 </div>
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest flex items-center gap-2"><Calendar className="w-3.5 h-3.5" /> INÍCIO</label>
                 ...
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest flex items-center gap-2"><Calendar className="w-3.5 h-3.5" /> FIM</label>
                       ...
                       {statsFrequenciaGeral.length > 0 ? statsFrequenciaGeral.map((row, idx) => {
                         ...
                         return (
                           <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-8 py-6 font-bold text-slate-700">{parseToDate(row.data)?.toLocaleDateString('pt-BR')}</td>
                              <td className="px-8 py-6 font-black text-blue-500 text-[10px] uppercase">{row.unidade}</td>
                              ...
                              <td className="px-8 py-6 text-right font-black text-slate-900">{pct}%</td>
                              <td className="px-8 py-6 text-xs text-slate-500">{row.tema || '--'}</td>
                           </tr>
                         );
                       }) : (
                         <tr><td colSpan={7} className="px-8 py-20 text-center text-slate-300 font-black uppercase">Nenhum registro encontrado.</td></tr>
                       )}
                    </tbody>
                 </table>
              </div>
